...
|---|---|---|---|
| **D — Yaseen** | character art + visual identity | **fifth machine, FULL CHECKPOINT PASS on `b51e3dc` — 15 ok / 0 failed / 5 N/A, dress rehearsal included (16 received, 10 spoken, 0 dropped).** Art itself not started yet. Clean setup needed `node`; getting there found **a hang in `checkpoint.sh` on any machine without node — see Blockers, it's A's file.** C's stale-binary check also caught me for real after a pull — worked exactly as intended. **✅ RECAP ART SHIPPED (`936c608`) — `walk-recap-01.mov`, 1.86MB of a 4MB budget, HEVC+alpha 1080×1920. Recap is no longer triage flipped; two of the three characters on stage tonight are no longer the same drawing.** Verified across all 241 frames, not one: residual old colour worst 0.57%, garment hue spread 5.4 (no strobing), alpha confirmed through **AVFoundation** (the dock's own decoder, not ffmpeg's). Checkpoint PASS 15 ok / 0 failed on the resolved tree, roster loads all five. Kept A's placeholder rows for researcher/analyst in the rebase. **A — `alpha_quality`, not `q:v`, is what costs bytes here: 0.9→0.65 halved the file at full resolution for 1.71/255 of edge fidelity. No downscale needed.** **✅ WORKSTREAM COMPLETE (`a217101`) — all five roles now have their own clip and the costume says what the role does: researcher in a lab coat, analyst in hi-vis, recap in warm orange. Your placeholders are replaced. No two characters on screen are the same drawing.** Roster loads five, zero `spoken but NOT shown`, checkpoint PASS 15 ok / 0 failed. **7.27MB of the 20MB budget.** **A — `walk-normal-01` is a committed SPARE referenced by nothing: the next crew member you add has a face on day one, just add a row pointing at it.** | nothing — workstream complete |
...
### D — A's placeholders are now real art, re-verified on the five-agent phrase

A: you wrote *"those two rows are yours the moment you have art — swap `asset`, drop
`mirrored`."* Done, with **one deliberate deviation**: I swapped both assets but **kept
`mirrored: true` on `researcher`**. Dropping it was right advice for a placeholder that
reused `walk-jazz-01`; now that researcher has its own clip the mirror is doing different
work — researcher is *derived* from jazz, so it shares scheduler's silhouette, and the
flip is what keeps the two readable side by side. `analyst` is bruce-derived and left
unflipped precisely so it does not read as `recap`, which is the mirrored bruce.

Re-ran your five-role phrase with the real art in place, not the stopgaps:

```
"clean up my inbox, schedule everything, research the Q3 rollout and analyse the numbers"

characters shown : triage · scheduler · researcher · analyst · recap
spoken but NOT shown : 0
received 26 · spoken 15 · SAY !! failures 0
```

Costumes now carry the role rather than just separating the characters: researcher in a
lab coat, analyst in hi-vis, recap in warm orange, triage and scheduler on the upstream
originals. C's five voices land on five different faces — Tessa and Rishi are no longer
Samantha *and* no longer someone else's drawing.

**A — one small gap, your file, and it is the reason nobody has seen four of these.**
`PHRASE` on line 15 of `run-demo.sh` is a plain assignment, not `${PHRASE:-...}`, so there
is **no way to run the five-agent crew without hand-rolling a curl**. The phrases are
documented in your table above, but documented is not runnable, and the capability that
best shows off the orchestrator is the one that takes the most effort to demonstrate. One
character:
...
runbook. Not on the demo path — the rehearsed phrase still routes to three — so this is
