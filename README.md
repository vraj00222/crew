...
**And when you walk away from the laptop, they phone you.** An agent that hits a decision
only you can make rings your actual phone, asks the question out loud, listens to your
answer — and the whole crew carries on from what you said.
...
And when one of those two really does need you — and you're not at the desk:
...
You never touched the laptop. The answer was transcribed, handed back to the agent that
asked, and **the recap closed the show knowing what you decided** — because a decision you
made out loud belongs to the whole crew, not to the one agent holding the phone.
...
The phone half is one command too — it stands up the call server, opens a tunnel, points
the number at it, and then *proves the whole path from outside* rather than assuming it:
...
> **Watching is only the answer while you're in the chair.**

The moment you leave the desk, a dock full of characters is worth nothing. Every agent
product solves this the same way: it queues the question and waits for you to come back.
So the work stops at exactly the point where a human was needed, and you find out an hour
later that nothing happened.

**Crew reaches you instead.** Three tools that touch a real phone:
...
The third one is the whole idea. An agent hits something only you can decide, and instead
of guessing or stopping:

1. It says on the dock what it's about to ask, **then** dials — so the room hears the
   question while your phone rings.
2. The call speaks the question, beeps, and listens.
3. You answer in a sentence and hang up.
4. The recording is transcribed and **deleted immediately** — the audio never lands on
   disk for more than the seconds it takes to read it. Only your words are kept.
5. Those words go back to the agent as the tool's result, *and* onto the task — so every
   agent that runs afterwards is handed your decision rather than re-guessing it.

The agent is blocked inside a single tool call for that whole conversation, which is why
it announces itself first: without that the dock would freeze for two minutes at exactly
the moment the interesting thing is happening in your hand.

**Consent is enforced by the API, not by our good intentions.** a1mobile only dials numbers
a human has OTP-verified, so the blast radius is exactly the phones we deliberately
verified — the OTP step is a CLI command, never a tool an agent can reach.

---

## How it works

Three processes on one machine, two HTTP interfaces, no message bus and no database — plus
a fourth process that only exists when the crew can phone you.
...
without waiting on each other — and one added later, additively, so nothing above changed
shape:
...
That third one is what keeps the stage alive while an agent is on the phone, and it's the
reason your spoken answer reaches the agents that run *after* the one you talked to.
...
¹ The live phone path — a real text, a real answered call, a real transcript — was proven
on hardware before this was merged. Everything above it in the table is proven *after*, on
a machine with no credentials at all, which is the point: the loop, the announcements, the
hand-off to later agents and the fallback all hold without a phone in the room. On the
demo Mac the one command that re-proves the real path end to end is
`./voiceos-bridge/mcp-server/phone.sh check` — it fetches our own webhook back *through the
public tunnel*, so a dead tunnel or a stale pointing can't pass.
...
- **The phone rang and the dock said nothing about it — twice, for two different reasons.**
  ...
  minutes, so there was simply no line to pace. Then, after the announcement was added,
  because a character is marked "signed off" when its `Done:` line is *queued*, not when
  it's *spoken* — and the model usually emits all its narration in one event. The
  announcement arrived a second later and was dropped by a guard meant for chatty agents.
  A real event now jumps the queue ahead of the sign-off instead of being filtered by it.
- **A test reported on a server it hadn't started.** `pkill` doesn't exist on Windows, so
  the old orchestrator kept :4001, the new one exited with `EADDRINUSE`, and the run
  "failed" against code that no longer existed. The same trap `test.sh` documents, walked
...
**Every irreversible thing got a rung below it.** `fake` for the agents, `phone-fake` for
the phone. The real phone needs five things up at once — team key, verified number, call
server, public tunnel, transcription — and a hackathon demo cannot be one bad wifi away
from having no third act. The simulated route depends on nothing outside the process,
answers the same question the same way every time, and drives the dock identically. Only
the ringing is missing, and only the operator knows.

**Announce before you dial, not after.** The agent is blocked for the whole call, so the
announcement has to be *sent* before the thing that blocks. Said afterwards it would be
narration; said first it's the reason the room knows what's being asked while the phone is
still ringing.
...
- **The phone needs a public tunnel, and a free tunnel's address changes every time.** An
  ...
  `phone.sh up` re-points the number on every start. Point it once by hand and the next
  run rings a phone that then says nothing — the worst failure we have.
- **A call can only reach a number someone OTP-verified**, which is a deliberate ceiling,
  not a limitation to work around. Verification is a CLI step a human does once; no agent
  can reach it.
- **Transcribing the answer is ours, not the carrier's.** a1mobile doesn't forward Telnyx's
  transcription callback, so the recording is fetched, read by OpenAI's API (~2s) or local
  whisper, and **deleted either way, success or failure**. Only the text survives, in a
  gitignored log. The call never claims to be recorded, because nothing is kept.
- **`crew_ask_user` can take up to two minutes**, and in `direct` mode the agent kill-timer
  now sizes itself around that automatically. If you override `AGENT_TIMEOUT_MS`, keep it
  above `CREW_ASK_WAIT_MS` or you'll SIGKILL an agent mid-conversation with a human.
